...
def predict_image(image, model, signal):
    ...
    img_array = np.array(img_array, copy=True)  # Ensure array is writable
    img = Image.fromarray(img_array)

    # Predict the image using the model
    results = model(img)

    # Skip saving results to avoid OpenCV error with readonly arrays
    # results.save('runs')
    ...
    # Select prediction based on number of detections
    if len(pred_list) == 0:
        # If no detections (excluding Bullseye), use the first detection from original results
        pred = df_results.iloc[0]
    ...
    # If more than 1 label is detected
            ...
            # If multiple predictions remain after filtering by confidence and area
                ...
                # If signal is 'C', choose the prediction that is central in the image
                    ...
                    if len([p for p in pred_shortlist if p['xmin'] > 250 and p['xmin'] < 774]) == 0:
